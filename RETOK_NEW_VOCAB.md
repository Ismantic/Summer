...
**问题**：`replace_tokenizer.py:134` 硬性要求新词表里有 `<pad>/<user>/<assistant>/<system>`
...
`replace_tokenizer.py` 会：为 81903 个 piece 逐个用 Qwen BBPE 编码、取均值映射到旧 embedding
...
Phase 1 用 v15 的混合（即"v8 mix" = `pretokenize_v7.py` 的 10 个源），Phase 2 anneal 用
`pretokenize_v12.py --mix anneal` 的 6 个源。两者并集：

| 数据集 | 用于 | 现成下载脚本 |
|---|---|---|
| FineWeb-Edu (sample-10BT) | P1+P2 | `download_data.py` |
| Wikipedia EN / CN | P1+P2 | 无，需自备 |
| Gutenberg | P1 | 无 |
| C4 EN / CN | P1 | 无 |
| CnnDailyMail / PeopleDaily | P1 | 无（旧为本地 txt） |
| SkyPile-150B | P1 | `download_data.py` |
| CCI3-HQ | P1 | 无 |
| Cosmopedia-v2 | P2 | `download_cosmopedia.py` |
| Chinese-FineWeb-Edu V2.2 | P2 | `download_chinese_fineweb_edu.py` |

**两个选项**：

- **严格复现 v15**：凑齐上面 v7 的全部 10 个源。最忠实，但部分源（Wikipedia/Gutenberg/C4/
  CnnDailyMail/PeopleDaily/CCI3-HQ）没有现成下载脚本，要自己找。
- **务实替代**：Phase 1 直接用 `pretokenize_v12.py --mix main`（9 个源，下载脚本齐全）。
  注意 v15 之所以比 v14 好就是因为用了 v7 mix 而非 v12 mix——换 v12 main mix 是近似，不是
  严格复现，但工程上省事很多。

> 下载后，**必须把 `pretokenize_v7.py` / `pretokenize_v12.py` 里 `SOURCES` 的 glob 路径
...
python pretokenize_v12.py --mix anneal \
    --tokenizer_model ./piece_v2.model \
    --cn_dict ./dict.txt \
...
新建 `run_v17_p1.sh`（从 `run_v15.sh` 复制，改三个路径）：
...
torchrun --nproc_per_node=2 --master_port=29517 finetune_muon.py \
    ...
    --inline_eval_cmd "bash inline_eval_vllm_v2.sh {step} $V17_P1 v17_p1 7500"
```

> 与 `run_v15.sh` 的唯一实质差异：`inline_eval` 改用 `inline_eval_vllm_v2.sh`
> （v15 原脚本用的 `inline_eval_progressive.sh` 已废弃，会受 transformers/vLLM 后端漂移影响，
> 见 `EVAL_README.md`）。
...
在 Phase 1 ckpt 上解冻、Aurora、anneal 数据、2000 步。新建 `run_v17_p2.sh`（从 `run_v16.sh`
复制）：
...
torchrun --nproc_per_node=2 --master_port=29518 finetune_muon.py \
    ...
    --inline_eval_cmd "bash inline_eval_vllm_v2.sh {step} $V17_P2 v17_p2 2000"
```
...
```bash
python prefetch_eval_datasets.py            # 一次性，拉 mmlu/ceval/gsm8k 数据集
bash smoke_new_tasks.sh                     # ~2min 冒烟（建议先指向 v17 ckpt）
python eval_with_piece_vllm.py --model_path output/phase2_ckpt_v17 \
    --tasks "lambada:0,piqa:5,hellaswag:10,arc_challenge:25,mmlu:5,ceval-valid:5,gsm8k:5"
python eval_analysis.py                     # 生成 % loss vs base 对比表
```

评估口径见 `EVAL_README.md`。注意：所有对比必须同一后端（统一 vLLM），不要混 transformers。
...
- **piece_tokenizer**：venv 里原先装的是 `/home/tfbao/ShiyuLab/Tokenizer` 的旧版绑定
  （`load()` 只接受单参数），与 `tokenizer_wrapper.py` 期望的 `load(model, cn_dict)`
...
- **cn-dict 不是纯加速**：验证发现它会**改变分词结果** —— 在词边界 pre-cut，避免 BPE
  ...
  `finetune_muon.py:_copy_tokenizer_artifacts` 会把 dict.txt 复制进每个 checkpoint，
