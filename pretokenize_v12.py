"""v12 pretokenizer — clean general data mix with HQ upgrades.

Phase 1 (broad HQ): 8 sources, EN:CN = 60:40, no code/math.
Phase 2 (concentrated HQ): 5 sources, narrower HQ tier.

Key additions vs v8 mix:
- Cosmopedia v2 (synthetic textbooks/stories, SmolLM-validated)
- Chinese-FineWeb-Edu V2.2 score 4-5 (OpenCSG quality-filtered CN edu)

Key removals:
- PeopleDaily, CnnDailyMail, C4_CN (low-signal redundant noise)
- All code/math (confirmed negative ROI on 0.6B)

Reuses pretokenize_v10.py's file-sharded multi-process machinery.
"""
import argparse
import glob
import gzip
...
SOURCE_DEFS = {
    ...
    "Cosmopedia": dict(format="parquet", field="text",
        glob="/mnt/data/Summer-data/cosmopedia-v2/cosmopedia-v2/train-*.parquet"),
    # ---- CN ----
    ...
    "CN_FineWeb_Edu": dict(format="parquet", field="text",
        glob="/mnt/data/Summer-data/Chinese-FineWeb-Edu-V2.2/4_5/*.parquet"),
}


# Phase 1: broad HQ mix, EN:CN = 60:40, drop low-signal sources
MAIN_WEIGHTS = {
    "FineWebEdu":     0.30,
    "Cosmopedia":     0.13,
    "Wikipedia_EN":   0.10,
...
# Phase 2 anneal: concentrated HQ tier, drop general web sources
ANNEAL_WEIGHTS = {
    "FineWebEdu":     0.20,
    "Cosmopedia":     0.25,
    "Wikipedia_EN":   0.15,
...
def build_shards(weights, total_tokens, n_workers, tmpdir, seq_len, max_line_chars,
    ...
    for src_name, w in weights.items():
        ...
        print(f"  {src_name:18s} w={w:.2f} files={len(files):4d} "
...
def main():
    ...
    tmpdir = tempfile.mkdtemp(prefix=f"pretok_v12_{args.mix}_")
