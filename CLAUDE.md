...
Run versions are tracked as `v7`…`v19` (see `runs/run_v*.sh`). **Current overall SOTA: `v18_p2_tie`** — Qwen3-1.7B + new 81903-piece tokenizer + LoRA tie-safe Phase 2 (1500 steps). WMT22 zh-en BLEU **20.46** vs base **22.34** (−1.88). Ckpt: `output/phase2_ckpt_v18_tie/checkpoint-1500/` (self-contained: `piece.model` + `dict.txt` + `adapter_model.safetensors` + `token_mapping.json`). Eval artifacts: `eval_results/full/v18_p2_tie_vllm/` + `eval_results/translate_wmt22/v18_p2_tie_*.json`.

Earlier milestones (kept for context): Phase 1 SOTA `v15` and P2 anneal `v16` on Qwen3-0.6B; `v17` switched to Qwen3-1.7B + LoRA; `v18` added the new 81903 piece + tie-safe LoRA. `v19` is an unrun from-scratch experiment. `docs/plans/autoresearch.md` is an autonomous-research loop spec for iterating on Phase 2.
...
make replace     # tools/replace_tokenizer.py: tokenizer + embedding swap -> *-new-tok/
make eval-base   # ReTok Table 2 benchmarks on base BBPE (lm_eval CLI)
make eval-new    # same on swapped piece tokenizer (evals/eval_with_piece.py adapter)

# --- Training (a Phase runs via torchrun on 2 GPUs) ---
bash runs/run_v16.sh  # each run_v*.sh is a self-contained hyperparameter script

# --- Evaluation (vLLM pipeline — see docs/eval/pipeline.md) ---
python evals/prefetch_eval_datasets.py        # one-time: pull mmlu/ceval/gsm8k datasets
bash runs/smoke_new_tasks.sh            # ~2min sanity check (--limit 30)
python evals/retro_eval_mono_vllm.py --gpus 0 1   # full retro sweep, ckpts sharded across GPUs
python evals/eval_analysis.py                 # build the % loss vs base comparison table
...
python evals/eval_with_piece_vllm.py --model_path <ckpt> --tasks "lambada:0,piqa:5,mmlu:5"
```

There is no test suite — `runs/smoke_test.sh` / `runs/smoke_new_tasks.sh` are end-to-end harness sanity checks (small `--limit`), not unit tests.

## Architecture

**`tools/replace_tokenizer.py`** — the irreversible surgery. `build_embedding_mapping` maps each new-vocab piece to old BBPE embeddings: one-to-one (~73.5%), multi-to-one averaged, or mean-vector fallback. Writes `piece.model`, `token_mapping.json`, resized weights. Changing this mapping invalidates every existing checkpoint.

**`train/finetune_muon.py`** — the trainer (the file you touch most for experiments). `--mode clm` for pretraining, `sft` for JSONL. Optimizer split: params with `ndim >= 2` and not `embed`/`lm_head` go to Muon/Aurora; embed + head go to an auxiliary AdamW (`SingleDeviceMuonWithAuxAdam` from `core/muon.py`). `--freeze_transformer` collapses this to plain AdamW on embed+head only (Phase 1). `--freeze_mapped_embeds` zeros grads for one-to-one-mapped rows via a backward hook. `--inline_eval_cmd` runs an eval script at every save with `{step}` substituted. LR schedule: `cosine`/`linear` with `--min_lr_ratio` (never decay to 0 — that was a known pathology).

**`core/muon.py` / `core/aurora.py`** — local optimizers. Aurora is a leverage-uniform polar variant of Muon (`--use_aurora`); per `docs/plans/autoresearch.md` it is the default for any Phase-2-style unfrozen training. `--moonshot_scaling` adds Moonshot's per-param LR scaling, orthogonal to the optimizer choice.

**`core/tokenizer_wrapper.py`** — `PieceTokenizerWrapper`, an HF-`PreTrainedTokenizer`-compatible shim over the `piece_tokenizer` C++ binding. Used everywhere the piece model meets HF/lm-eval code.

**`data_prep/pretokenize*.py`** — pack raw text into `[N, seq_len]` int32 tensors saved as `.pt`. Versioned (`_v7`, `_v9`, `_v10`, `_v12`) — each version is a different data mix / sampler. `data_prep/download_*.py` fetch source corpora (Cosmopedia, Chinese-FineWeb-Edu, MAP-CC, etc.).

**Eval pipeline** — `evals/eval_with_piece_vllm.py` is the primary entry: a `lm-eval-harness` `TemplateLM` subclass backed by vLLM, auto-detecting piece vs HF tokenizer. `evals/eval_pretrain_translate_vllm.py` does WMT22 BLEU+COMET. `evals/retro_eval_mono_vllm.py` / `evals/retro_eval_vllm.py` are batch drivers (`--gpus 0 1` shards checkpoints across GPUs). `evals/eval_analysis.py` aggregates into the final table. See `docs/eval/pipeline.md` for the deprecated non-vLLM files and why they were superseded.
...
- The eval harnesses (`eval_pretrain_translate*.py`, `eval_ppl.py`, `eval_with_piece*.py`, `runs/smoke_test.sh`) are **ground truth — do not edit them to make results look better.**
