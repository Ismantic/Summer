...
if [ ! -f "$MAIN_PT" ]; then
  ...
  cd $SUMMER && $PYTHON pretokenize_v12.py \
    --mix main \
...
INLINE_EVAL_P1="bash $SUMMER/inline_eval_progressive.sh {step} $V12_P1 v12_p1 5000"
cd $SUMMER && $TORCHRUN --nproc_per_node=2 --master_port=29505 finetune_muon.py \
    --model_path $NEW_TOK \
    --train_data $MAIN_PT \
    --mode clm \
    --freeze_transformer \
...
INLINE_EVAL_P2="bash $SUMMER/inline_eval_progressive.sh {step} $V12_P2 v12_p2 2000"
cd $SUMMER && $TORCHRUN --nproc_per_node=2 --master_port=29505 finetune_muon.py \
