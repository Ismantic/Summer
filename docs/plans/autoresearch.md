...
2. **Read in-scope files**:
   ...
   - `core/muon.py` — Muon optimizer
   - `core/aurora.py` — Aurora optimizer (drop-in for Muon)
   - `data_prep/pretokenize_v7.py` — data sampler (only touch if doing v9+ Phase 1)
   - `evals/eval_pretrain_translate.py`, `evals/eval_ppl.py`, `runs/smoke_test.sh` — eval
...
**You CAN modify:**
- `train/finetune_muon.py` — training loop, optimizer flags
- `core/muon.py`, `core/aurora.py` — optimizer internals
- `runs/run_v8_s2_*.sh` — hyperparameter scripts (one per experiment)
- `data_prep/pretokenize_v7.py` SOURCES list & weights — for v9+ Phase 1 data mix changes
- New scripts for new ideas (e.g. KL-regularized Phase 2)

**You CANNOT modify:**
- `evals/eval_pretrain_translate.py`, `evals/eval_ppl.py`, `runs/smoke_test.sh`, `evals/eval_with_piece.py` — eval harnesses are ground truth
- Base model paths, test data paths
- The token mapping in `tools/replace_tokenizer.py` (would invalidate ckpts)
...
LOOP FOREVER:
  ...
  3. Implement: edit train/finetune_muon.py / core/aurora.py / runs/run_v8_s2_*.sh.
     Commit with a clear one-line message: `expN: <what changed>`
  4. Launch in background:
       bash runs/run_v8_s2_<tag>.sh > output/<tag>_run.log 2>&1
...
Every `runs/run_v8_s2_<tag>.sh` should produce a tag-named eval artifact set:
```
