...
第 3 条和似然协议下 S0 的 ARC-Easy 0.5564 放在一起,说明的是第三种情况:
**知识信号存在,但模型没法把它转成一个字母答案** —— 不是不会格式,也不是完全
没知识,是这两者没接上。nanochat 能接上,一部分原因是它 SFT 直接训了 ARC 的
train split(`chat_sft.py:88-89`),我们的 SFT 混比里选择题是零(MC 数据只在
midtrain,而 midtrain 整段算 loss,「答字母」这个信号被稀释)。
...
同一档。头号嫌疑是输出侧学习率:它 lm_head 独立、`unembedding_lr` 0.008,
我们 tie 到嵌入、压在 AdamW 组的 3e-4 里(慢 27 倍)。实测 `<end>` 那一行
从 S0 到 SFT 的位移只排 **9921/81903**。这条还没验证。
