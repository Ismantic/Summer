...
上面那 65% 是**教师强制**的测量:给一段良好的回答,问模型「这里会停吗」。
换成自由生成(用模型自己的输出当上下文),4 条测试 **0 条**在 80 token 内停下:

```
1+1 等于几?  → 1+1=2  然后 #### 1+1=2  #### 1+1=2  #### 1+1=2 …
光合作用     → 第一句正确,然后「是植物进行呼吸作用的主要方式」重复三遍
你好你是谁?   → 跑题到自闭症,整段重复
```

**它先退化成复读,而复读没有自然的结束点** —— 停止符学会了,但轮不到它用。

所以停止符那个问题解决了,剩下的是**复读**,而复读和 MMLU 差 0.24 是同一个
根因:预训练 13B token 太少。这一步不是 SFT 或数据格式能补的。

> **教师强制的探针不能预测自由生成的行为。** 这是这条线上最值得记的方法论
> 教训:P(停止符) 从 0.325 升到 0.595 是真的,但它不等于「模型会正常收尾」。
> 要量后者只能自由生成着数。
