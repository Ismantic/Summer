"""Muon / Aurora 优化器。**只依赖 torch。**

两套更新规则,通过 `SingleDeviceMuonWithAuxAdam(update_fn=...)` 二选一:

- `muon_update`   —— Newton-Schulz 五次迭代求正交化更新(Keller Jordan)
- `aurora_update` —— leverage-uniform polar(Tilde Research, 2026),对高瘦矩阵
                     防「神经元死亡」。方阵上退化为标准 Muon

v18 SOTA 用的是 Aurora:`--use_aurora` → `update_fn=aurora_update`,
优化 3,211,264 个 LoRA 参数(q_proj/v_proj, r=16),embed 的 167,737,344 走辅助
AdamW。注意 Aurora 在「1.7B + LoRA」这个配置下**没有做过消融** —— 唯一那次
对照(v8_s2_aurora vs v8_s2_muon5e5,0.6B 全参数 500 步)结论是打平。详见
docs/WHY.md。

来源:
  Muon    https://github.com/KellerJordan/modded-nanogpt
  Aurora  https://github.com/tilde-research/aurora-release
"""
import torch
...
class MuonWithAuxAdam(torch.optim.Optimizer):
    ...
    from core.muon import MuonWithAuxAdam
...
# --------------------------------------------------------------------
# Aurora:leverage-uniform polar,muon_update 的 drop-in 替代
# --------------------------------------------------------------------
