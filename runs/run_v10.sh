...
INLINE_EVAL_P1="bash $SUMMER/runs/inline_eval_progressive.sh {step} $V10_P1 v10_p1 5000"
cd $SUMMER && $TORCHRUN --nproc_per_node=2 --master_port=29503 train/finetune_muon.py \
    --model_path $NEW_TOK \
    --train_data $MAIN_PT \
    --mode clm \
    --freeze_transformer \
...
bash $SUMMER/runs/eval_full.sh phase1_ckpt_v10_final
...
INLINE_EVAL_P2="bash $SUMMER/runs/inline_eval_progressive.sh {step} $V10_P2 v10_p2 2000"
cd $SUMMER && $TORCHRUN --nproc_per_node=2 --master_port=29503 train/finetune_muon.py \
...
bash $SUMMER/runs/eval_full.sh phase2_ckpt_v10_final
