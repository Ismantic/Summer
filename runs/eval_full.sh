...
#
# Usage:
#   bash runs/eval_full.sh <ckpt_tag> [<ckpt_tag2> ...]
# where each <ckpt_tag> is the name of a directory under output/.
# Example:
#   bash runs/eval_full.sh phase1_ckpt_v8 phase2_ckpt_v8_s2_aurora
#
...
eval_new() {
    ...
    CUDA_VISIBLE_DEVICES=$device $PYTHON -u $SUMMER/evals/eval_pretrain_translate.py \
        --model_path "$ckpt" --testset wmt22 --exemplar_set wmt21 --direction both \
        --num_fewshot 5 --max_samples $BLEU_SAMPLES --batch_size 16 \
        --output_path "$out/wmt22.json" > "$out/wmt22.log" 2>&1 || true
    CUDA_VISIBLE_DEVICES=$device $PYTHON -u $SUMMER/evals/eval_ppl.py \
        --model_path "$ckpt" --valid_pt $VALID_PT --batch_size 16 \
        --output_path "$out/ppl.json" > "$out/ppl.log" 2>&1 || true
}

if [ "$#" -eq 0 ]; then
    echo "usage: bash runs/eval_full.sh <ckpt_tag> [<ckpt_tag2> ...]"
    exit 1
fi
